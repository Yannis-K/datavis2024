...
export function getTimelineData2(data) {
  // Créer un tableau pour stocker les éléments de la timeline
  const timelineItems = [];
  ...
  data.forEach((laureate) => {
    ...
    const timelineItem = {
      id: uniqueId,
      //content: `${laureate.full_name} - ${laureate.prize}`,
      content: `<div><img src="${laureate.imageUrl}" alt="${laureate.full_name}" style="max-width: 100px; max-height: 100px;">${laureate.full_name} (${laureate.birth_date} - ${laureate.death_date}) </div>`,

      //start: laureate.birth_date,
      start: laureate.birth_date,
      end: laureate.death_date,
      type: "box",
    };
